...
function createBuiltInProvider(providerId: string, config: BuiltInProviderConfig): AIProvider {
    switch (providerId) {
...
export class AuraChat extends LitElement {
    ...
    private async _streamAIResponse() {
        ...
        try {
            const systemPrompt = await promptBuilder.build(this._config!.behavior);
            const request = {
                systemPrompt,
                messages: this._messages,
